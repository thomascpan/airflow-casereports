...
def extract_pubmed_data() -> None:
    ...
    prefix = s3file + '/pubmed/original'

    # deleting old entries in the bucket
    dest_path = s3file + '/pubmed/original/'
    wildcard = s3file + '/pubmed/original/*.*'
...
def transform_pubmed_data() -> None:
    ...
    root_dir = '/usr/local/airflow'
    temp_dir = os.path.join(root_dir, 'temp')
    src_path = s3file + '/pubmed/original/'
    dest_path = s3file + '/pubmed/json/'
    src_bucket_name = 'supreme-acrobat-data'
    dest_bucket_name = 'supreme-acrobat-data'
    wildcard_key = s3file + '/pubmed/original/*.*'
...
def update_mongo() -> None:
    ...
    """
    src_path = s3file + '/pubmed/json/'
    src_bucket_name = 'supreme-acrobat-data'
    wildcard_key = s3file + '/pubmed/json/*.*'
